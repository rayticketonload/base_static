...
;(function (root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        define(['jquery', 'webuploader/webuploader'], factory);
    ...
    Uploader.DEFAULTS = {
        ...
        fileVal: 'File', // 文件参数名
        toolbar: true,
        filetypes: "jpg,jpeg,bmp,png,gif,pdf,doc,docx,xls,xlsx", //文件类型
        //fileType: 'file', // file,images
    ...
    Uploader.prototype = {
        ...
        __listenUploader: function(){
            ...
            uploader.on('error', function (type) {
                switch (type) {
                    case 'Q_EXCEED_NUM_LIMIT':
                        opts.debug("错误：上传文件数量过多！");
                        break;
                    case 'Q_EXCEED_SIZE_LIMIT':
                        opts.debug("错误：文件总大小超出限制！");
                        break;
                    case 'F_EXCEED_SIZE':
                        opts.debug("错误：文件大小超出限制！");
                        break;
                    case 'Q_TYPE_DENIED':
                        opts.debug("错误：禁止上传该类型文件！");
                        break;
                    case 'F_DUPLICATE':
                        opts.debug("错误：请勿重复上传该文件！");
                        break;
                    default:
                        opts.debug('错误代码：' + type);
                        break;
                }
            });
            ...
            uploader.on('uploadError', function (file, reason) {
                ...
                opts.debug(file.name + "上传失败，错误代码：" + reason);
        ...
        __addFile: function(file){
            ...
            if ( file.getStatus() === 'invalid' ) {
                ...
                // @todo lazyload
                $wrap.text( '预览中' );
                if(/jpg|jpeg|png|webp|gif|bmp/g.test(file.ext)) {
                    ...
                    $img = '<div class="file-placeholder"></div>';
                    $wrap.empty().append($img);
                }
